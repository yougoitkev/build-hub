...
export function QuickActionCard({ label, description, icon: Icon, link, color = "text-primary bg-primary/10", delay = 0 }) {
  ...
  return (
    <button
      ...
      className={cn(
        "group relative flex flex-col items-start gap-2.5 p-4 rounded-[var(--radius-shell)] border border-border/40 bg-card/50 backdrop-blur-xl",
    ...
    >
      ...
      <div className={cn("relative h-10 w-10 rounded-[var(--radius-panel)] flex items-center justify-center shrink-0 border border-border/30 backdrop-blur transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3", color)}>
